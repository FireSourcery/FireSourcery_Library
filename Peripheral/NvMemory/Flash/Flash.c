...
static inline Flash_Status_T FinalizeWriteEraseCommon(Flash_T * p_flash)
{
	...
	{
		//parse error
		status = (HAL_Flash_ReadErrorProtectionFlag(p_flash->CONFIG.P_HAL) == true) ? FLASH_STATUS_ERROR_PROTECTION : FLASH_STATUS_ERROR_CMD;
	}
	else
	{
		status = (p_flash->IsVerifyEnable == true) ? FLASH_STATUS_START_VERIFY : FLASH_STATUS_SUCCESS;
	}
...
static inline Flash_Status_T FinalizeWrite(Flash_T * p_flash)
{
	...
	{
		if(NvMemory_ChecksumOp(p_flash) == false) { status = FLASH_STATUS_ERROR_CHECKSUM; }
...
static inline void SetWrite(Flash_T * p_flash)
{
	NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdWritePage, (FinalizeOp_T)FinalizeWrite);
	NvMemory_SetOpCmdSize(p_flash, FLASH_UNIT_WRITE_SIZE, 1U);
}

static inline void SetErase(Flash_T * p_flash)
{
	NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdEraseSector, (FinalizeOp_T)FinalizeErase);
	NvMemory_SetOpCmdSize(p_flash, FLASH_UNIT_ERASE_SIZE, 1U);
}

static inline void SetVerifyWrite(Flash_T * p_flash)
{
	NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdVerifyWriteUnit, (FinalizeOp_T)FinalizeVerify);
	NvMemory_SetOpCmdSize(p_flash, FLASH_UNIT_VERIFY_WRITE_SIZE, 1U);
}

static inline void SetVerifyErase(Flash_T * p_flash)
{
	NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdVerifyEraseUnits, (FinalizeOp_T)FinalizeVerify);
...
Flash_Status_T Flash_SetWriteOnce(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
	...
	if(status == FLASH_STATUS_SUCCESS)
	{
		NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdWriteOnce, (FinalizeOp_T)FinalizeWrite);
...
Flash_Status_T Flash_SetReadOnce(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size) //option to provide buffer?
{
	...
	if(status == FLASH_STATUS_SUCCESS)
	{
		NvMemory_SetOpFunctions(p_flash, (NvMemory_StartCmd_T)StartCmdReadOnce, (FinalizeOp_T)0U);  //todo FinalizeOp
...
{
	return ((Flash_SetWrite(p_flash, p_destFlash, p_source, size) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}
...
Flash_Status_T Flash_ContinueWrite_Blocking(Flash_T * p_flash, const uint8_t * p_source, size_t size)
{
	...
	if(status == FLASH_STATUS_SUCCESS) { p_flash->p_OpDest += size; }
...
{
	return ((Flash_SetErase(p_flash, p_destFlash, size) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}

Flash_Status_T Flash_VerifyWrite_Blocking(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
	return ((Flash_SetVerifyWrite(p_flash, p_destFlash, p_source, size) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}

Flash_Status_T Flash_VerifyErase_Blocking(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
{
	return ((Flash_SetVerifyErase(p_flash, p_destFlash, size) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}

Flash_Status_T Flash_WriteOnce_Blocking(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
	return ((Flash_SetWriteOnce(p_flash, p_destFlash, p_source, size) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}
...
{
	return ((Flash_SetOp(p_flash, p_destFlash, p_source, size, opType) == FLASH_STATUS_SUCCESS) ? NvMemory_ProcOp_Blocking(p_flash) : FLASH_STATUS_ERROR_INPUT);
}

// proc last set
// Flash_Status_T Flash_ProcThis_Blocking(Flash_T * p_flash)
// {
// 	return NvMemory_ProcOp_Blocking(p_flash);
// }
