...
static Flash_Status_T SetWrite(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
    return NvMemory_SetOpControl(p_flash, &FLASH_OP_WRITE, p_destFlash, p_source, size);
}
...
static Flash_Status_T SetErase(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
{
    p_flash->IsOpBuffered = false;
    return NvMemory_SetOpControl(p_flash, &FLASH_OP_ERASE, p_destFlash, 0U, size);
}
...
static Flash_Status_T SetVerifyWrite(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
    return NvMemory_SetOpControl(p_flash, &FLASH_OP_VERIFY_WRITE, p_destFlash, p_source, size);
}
...
static Flash_Status_T SetVerifyErase(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
{
    return NvMemory_SetOpControl(p_flash, &FLASH_OP_VERIFY_ERASE, p_destFlash, 0U, size);
}
...
static Flash_Status_T SetWriteOnce(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
    return NvMemory_SetOpControl(p_flash, &FLASH_OP_WRITE_ONCE, p_destFlash, p_source, size);
}
...
static Flash_Status_T SetReadOnce(Flash_T * p_flash, uint8_t * p_dataResult, const uint8_t * p_destOnce, size_t size)
{
    Flash_Status_T status = NvMemory_SetOpDestination(p_flash, p_destOnce, size);
...
Flash_Status_T Flash_SetOp(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_data, size_t size, Flash_Operation_T opId)
{
    ...
    switch(opId)
    {
        case FLASH_OPERATION_WRITE:             status = SetWrite(p_flash, p_destFlash, p_data, size);                break;
        case FLASH_OPERATION_ERASE:             status = SetErase(p_flash, p_destFlash, size);                        break;
        case FLASH_OPERATION_VERIFY_WRITE:      status = SetVerifyWrite(p_flash, p_destFlash, p_data, size);          break;
        case FLASH_OPERATION_VERIFY_ERASE:      status = SetVerifyErase(p_flash, p_destFlash, size);                  break;
        case FLASH_OPERATION_WRITE_ONCE:        status = SetWriteOnce(p_flash, p_destFlash, p_data, size);            break;
        case FLASH_OPERATION_READ_ONCE:         status = SetReadOnce(p_flash, (uint8_t *)p_data, p_destFlash, size);  break;
...
{
    Flash_Status_T status = ProcAfterSet(p_flash, SetWrite(p_flash, p_destFlash, p_source, size));
...
Flash_Status_T Flash_StartContinueWrite(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
{
    return SetWrite(p_flash, p_destFlash, 0, size);
}
...
{
    return ProcAfterSet(p_flash, SetErase(p_flash, p_destFlash, size));
}
...
Flash_Status_T Flash_VerifyWrite_Blocking(Flash_T * p_flash, const uint8_t * p_destFlash, const uint8_t * p_source, size_t size)
{
    ...
    return ProcAfterSet(p_flash, SetVerifyWrite(p_flash, p_destFlash, p_source, size));
    ...
    return (memcmp(p_destFlash, p_source, size) == 0U) ? NV_MEMORY_STATUS_SUCCESS : NV_MEMORY_STATUS_ERROR_VERIFY;
#endif
}

Flash_Status_T Flash_VerifyErase_Blocking(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
{
    return ProcAfterSet(p_flash, SetVerifyErase(p_flash, p_destFlash, size));
}
...
{
    Flash_Status_T status = ProcAfterSet(p_flash, SetWriteOnce(p_flash, p_destFlash, p_source, size));
...
{
    return ProcAfterSet(p_flash, SetReadOnce(p_flash, p_dataResult, p_destOnce, size));
}
...
// {
//     p_flash->Status = (SetWrite(p_flash, p_destFlash, p_source, size) == NV_MEMORY_STATUS_SUCCESS ? NvMemory_StartOp(p_flash) : NV_MEMORY_STATUS_ERROR_INPUT);
//     return p_flash->Status;
// }

// Flash_Status_T Flash_StartErase_NonBlocking(Flash_T * p_flash, const uint8_t * p_destFlash, size_t size)
// {
//     p_flash->Status = (SetErase(p_flash, p_destFlash, size) == NV_MEMORY_STATUS_SUCCESS ? NvMemory_StartOp(p_flash) : NV_MEMORY_STATUS_ERROR_INPUT);
//     return p_flash->Status;
// }
