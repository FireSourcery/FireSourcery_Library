...
{
    HAL_ADC_Activate(p_analog->CONST.P_HAL_ADC, p_conversion->PIN);
...
static inline void WriteAdcFifo(Analog_T * p_analog)
{
    ...
    HAL_ADC_WriteFifoCount(p_analog->CONST.P_HAL_ADC, p_analog->ActiveChannelCount);
    ...
    for(uint8_t iConversion = 0U; iConversion < p_analog->ActiveChannelCount - 1U; iConversion++)
    {
        ...
        HAL_ADC_WriteFifoPin(p_analog->CONST.P_HAL_ADC, p_conversion->PIN);
    }
    Ring_PeekIndex(&p_analog->ConversionQueue, &p_conversion, p_analog->ActiveChannelCount);
    HAL_ADC_ActivateFifo(p_analog->CONST.P_HAL_ADC, p_conversion->PIN);
}
#endif
...
{
    // xor with previous
    if(p_options->FLAGS.HwTriggerConversion == 1U)      { HAL_ADC_EnableHwTrigger(p_analog->CONST.P_HAL_ADC); }
    else                                                { HAL_ADC_DisableHwTrigger(p_analog->CONST.P_HAL_ADC); }
#ifdef CONFIG_ANALOG_HW_CONTINOUS_CONVERSION_ENABLE
    if(p_options->FLAGS.ContinuousConversion == 1U)     { HAL_ADC_EnableContinuousConversion(p_analog->CONST.P_HAL_ADC): }
    else                                                { HAL_ADC_DisableContinuousConversion(p_analog->CONST.P_HAL_ADC); }
#endif
...
{
    HAL_ADC_Init(p_analog->CONST.P_HAL_ADC);
