...
static inline void Chip_DeinitLpuart(uint8_t lpuartId, uint8_t clockSource, PORT_Type * p_port, uint8_t pinTx, uint8_t pinRx)
{
	uint8_t pccIndex = lpuartId + PCC_LPUART0_INDEX;
...
//	PCC->PCCn[p_config->PORT_INDEX] &= ~PCC_PCCn_CGC_MASK; //defer deinit to cpu denit to avoid deinit shared port pins
}
