...
static inline bool HAL_Serial_ReadIsTxRegEmpty(const HAL_Serial_T * p_hal) { return (((p_hal->STAT & LPUART_STAT_TDRE_MASK) != 0U) ? true : false); }
static inline bool HAL_Serial_ReadIsRxRegFull(const HAL_Serial_T * p_hal) { return (((p_hal->STAT & LPUART_STAT_RDRF_MASK) != 0U) ? true : false); }
...
static inline bool HAL_Serial_ReadRxOverrun(HAL_Serial_T * p_hal) { return ((p_hal->STAT & LPUART_STAT_OR_MASK) != 0U) ? true : false; }
...
static inline void HAL_Serial_WriteTxSwitch(HAL_Serial_T * p_hal, bool enable)
{
	...
	while ((bool)((p_hal->CTRL & LPUART_CTRL_TE_MASK) != 0U) != enable) {}	/* Wait for the register write operation to complete */
}
...
static inline void HAL_Serial_WriteRxSwitch(HAL_Serial_T * p_hal, bool enable)
{
    ...
    while((bool)((p_hal->CTRL & LPUART_CTRL_RE_MASK) != 0U) != enable) {}    /* Wait for the register write operation to complete */
}
