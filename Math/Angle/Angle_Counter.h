...
*/
static inline int32_t _speed_fract16_of_counter_freq(uint32_t speed_per_count, int32_t freqD) { return freqD * speed_per_count >> 15; }
static inline uint32_t _angle_speed_of_counter_freq(uint32_t angle32_per_count, int32_t freqD) { return freqD * angle32_per_count >> ANGLE_EXT_SHIFT; }
...
static inline int32_t Angle_Counter_GetRpm(const Angle_Counter_T * p_counter) { return rpm_of_count_freq(p_counter->FreqD, p_counter->Ref.CountsPerRevolution); }
static inline int32_t Angle_Counter_GetCps(const Angle_Counter_T * p_counter) { return cps_of_count_freq(p_counter->FreqD, p_counter->Ref.CountsPerRevolution); }
