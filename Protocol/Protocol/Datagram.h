...
typedef struct
{
	...
	uint8_t * const P_NONVOLATILE_BUFFER; 	//store var if non  write


	void (*const BUILD_TX_HEADER) (void * p_txPacketHeader, const void * p_datagram);
	//	void (*const BUILD_HEADER) (const void * p_txPacket, uint8_t dataLength, P_RX_CMD );
//headerlength known header length alleviates additional memcopy
	...
	void (*const OnComplete)(void * p_datagram);
	 ...
	 uint8_t DatagramVarCount; //active
	...
	const uint8_t HeaderSize;
	 bool DatagramModeEnable;  //periodic / on signal
