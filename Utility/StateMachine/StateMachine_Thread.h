...
/******************************************************************************/
static inline void _StateMachine_Synchronous_Thread(StateMachine_Active_T * p_fields, void * p_context)
{
    if (_StateMachine_AcquireAsyncIsr(p_fields) == true) /* Disabled when input is processing. ensure any transition is completed */
    {
        _StateMachine_ProcState(p_fields, p_context);
        _StateMachine_ReleaseAsyncIsr(p_fields);
    }
}
