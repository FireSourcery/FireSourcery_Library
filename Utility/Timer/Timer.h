...
typedef struct
{
	volatile uint32_t * const P_BASE; /* Base Timer */
...
static inline void Timer_Zero(Timer_T *p_timer) 	{p_timer->TimeRef = *p_timer->CONFIG.P_BASE = 0U;}
...
static inline bool Timer_GetComplete(Timer_T * p_timer) 		{return (Timer_GetElapsed_Ticks(p_timer) >= p_timer->Period) ? true : false;}
static inline bool Timer_CheckComplete(Timer_T * p_timer) 		{return Timer_GetComplete(p_timer);}
...
static inline void Timer_Init(Timer_T * p_timer)
{
	p_timer->IsOneShot = false;
	p_timer->Period = 0U;
}

static inline void Timer_InitOneShot		(Timer_T * p_timer) 					{p_timer->IsOneShot = true; }//set period , onshot count remining = 0
