...
/*
    ...
    on the handler's behalf - Packet_Xfer_T has no step field, and P_SUB_STATE is never
...
/******************************************************************************/
/*!
    Sub-state - the cursor, held in the socket's P_SUB_STATE buffer.
*/
/******************************************************************************/
/*!
...
*/
typedef enum Protocol_DataMode_StateId
{
    ...
    PROTOCOL_DATA_MODE_STATE_ERROR,     /* Opening request too short to be one - report and close */
    PROTOCOL_DATA_MODE_STATE_OVERRUN,   /* A chunk past the declared size - report and close */
}
...
static inline Protocol_DataMode_StateId_T _DataMode_OpenStateOf(const Packet_Meta_T * p_rxMeta)
{
...
static inline Protocol_ReqCode_T Protocol_DataMode_Open(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const Protocol_DataMode_Req_T * p_req, Protocol_DataMode_Resp_T * p_resp)
{
    ...
    return Protocol_DataMode_Reply(p_xfer->p_TxMeta, p_resp, p_state->ReqId, p_state->Status, (p_state->Size == 0U) ? PROTOCOL_REQ_DONE : PROTOCOL_REQ_RESPOND);
}
...
static inline Protocol_ReqCode_T Protocol_DataMode_ReadData(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const void * p_rx, uint8_t * p_chunk)
{
    ...
    return (Protocol_DataMode_ReadChunk(p_app, p_state, p_xfer->p_TxMeta, p_chunk) == PROTOCOL_DATA_MODE_STATUS_OK)
         ...
         : Protocol_DataMode_Reply(p_xfer->p_TxMeta, (Protocol_DataMode_Resp_T *)p_chunk, p_state->ReqId, p_state->Status, PROTOCOL_REQ_DONE);
}
...
static inline Protocol_ReqCode_T Protocol_DataMode_WriteData(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const uint8_t * p_chunk, Protocol_DataMode_Resp_T * p_resp)
{
    ...
    return (Protocol_DataMode_WriteChunk(p_app, p_state, p_xfer->p_RxMeta, p_chunk) == PROTOCOL_DATA_MODE_STATUS_OK)
         ? PROTOCOL_REQ_ACCEPT
         : Protocol_DataMode_Reply(p_xfer->p_TxMeta, p_resp, p_state->ReqId, p_state->Status, PROTOCOL_REQ_DONE);
}
...
static inline Protocol_ReqCode_T Protocol_DataMode_WriteClose(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const uint8_t * p_chunk, Protocol_DataMode_Resp_T * p_resp)
{
    ...
    return Protocol_DataMode_Reply(p_xfer->p_TxMeta, p_resp, p_state->ReqId, status, PROTOCOL_REQ_DONE);
}
...
static inline Protocol_ReqCode_T Protocol_DataMode_Malformed(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const void * p_rx, Protocol_DataMode_Resp_T * p_resp)
{
    ...
    return Protocol_DataMode_Reply(p_xfer->p_TxMeta, p_resp, p_xfer->p_RxMeta->Id, PROTOCOL_DATA_MODE_STATUS_MALFORMED, PROTOCOL_REQ_DONE);
}
...
static inline Protocol_ReqCode_T Protocol_DataMode_Read(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const void * restrict p_rxPayload, void * restrict p_txPayload)
{
    ...
    switch (p_state->StateId)
    {
        ...
        case PROTOCOL_DATA_MODE_STATE_OVERRUN:  /* unreachable - a Read absorbs no chunks */
...
static inline Protocol_DataMode_StateId_T _DataModeWrite_StateOf(Protocol_DataMode_Interface_T * p_app, const Protocol_DataMode_State_T * p_state, const Packet_Meta_T * p_rxMeta)
{
    ...
    /* A chunk past the declared size is the host misbehaving, not a memory fault. Distinct
       from ERROR: the transfer is open, so its reply is labelled and statused differently. */
    if ((size_t)p_rxMeta->Length > (p_state->Size - p_state->Count)) { return PROTOCOL_DATA_MODE_STATE_OVERRUN; }
...
static inline Protocol_ReqCode_T Protocol_DataMode_Write(Protocol_DataMode_Interface_T * p_app, Packet_Xfer_T * p_xfer, const void * restrict p_rxPayload, void * restrict p_txPayload)
{
    ...
    p_state->StateId = _DataModeWrite_StateOf(p_app, p_state, p_xfer->p_RxMeta);
    ...
    switch (p_state->StateId)
    {
        ...
        case PROTOCOL_DATA_MODE_STATE_ERROR:    return Protocol_DataMode_Malformed (p_app, p_xfer, p_rxPayload, p_txPayload);
        case PROTOCOL_DATA_MODE_STATE_OVERRUN:  return Protocol_DataMode_Overrun   (p_app, p_xfer, p_rxPayload, p_txPayload);
        case PROTOCOL_DATA_MODE_STATE_IDLE:
        default:                                return Protocol_DataMode_Idle      (p_app, p_xfer, p_rxPayload, p_txPayload);
    }
}
