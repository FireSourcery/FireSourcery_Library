...
*/
/******************************************************************************/
typedef struct PACKET_PACKED Protocol_FlashLoader_Req  { uint32_t Address; uint32_t Size; uint32_t Config; } Protocol_FlashLoader_Req_T;
typedef struct PACKET_PACKED Protocol_FlashLoader_Resp { uint16_t Status; }                                  Protocol_FlashLoader_Resp_T;
...
static inline Protocol_ReqCode_T Protocol_FlashLoaderDirect_Reply(const Protocol_FlashLoader_State_T * p_state, Packet_Xfer_T * p_xfer, void * p_txPayload, uint16_t status, Protocol_ReqCode_T onOk)
{
    ((Protocol_FlashLoader_Resp_T *)p_txPayload)->Status = status;
    p_xfer->p_TxMeta->Id     = p_state->ReqId;
    p_xfer->p_TxMeta->Length = sizeof(Protocol_FlashLoader_Resp_T);
...
*/
static inline Protocol_ReqCode_T Protocol_FlashLoaderDirect_Open(Protocol_FlashLoader_T * p_app, Protocol_FlashLoader_State_T * p_state, Packet_Xfer_T * p_xfer, const void * p_rxPayload, void * p_txPayload)
{
    const Protocol_FlashLoader_Req_T * p_req = p_rxPayload;

    if (p_xfer->p_RxMeta->Length < sizeof(Protocol_FlashLoader_Req_T))
    {
    ...
    p_state->Index   = 0U;
    p_state->ReqId   = p_xfer->p_RxMeta->Id;
    p_state->Status  = (uint16_t)Flash_SetContinueWrite(p_app->P_FLASH, p_state->Address, p_state->Size);

    /* Nothing to stream: the opening status is also the closing one. */
    return Protocol_FlashLoaderDirect_Reply(p_state, p_xfer, p_txPayload, p_state->Status,
...
static inline Protocol_ReqCode_T Protocol_FlashLoaderDirect_Read(void * p_context, Packet_Xfer_T * p_xfer, const void * restrict p_rxPayload, void * restrict p_txPayload)
{
    ...
    Protocol_FlashLoader_State_T * p_state = p_xfer->p_Substate;
    ...
    {
        return Protocol_FlashLoaderDirect_Open(p_app, p_state, p_xfer, p_rxPayload, p_txPayload);
    }

    if (p_state->Index >= p_state->Size)
    {
...
*/
/******************************************************************************/
static inline Protocol_ReqCode_T Protocol_FlashLoaderDirect_Write(void * p_context, Packet_Xfer_T * p_xfer, const void * restrict p_rxPayload, void * restrict p_txPayload)
{
    Protocol_FlashLoader_T * p_app = p_context;
    Protocol_FlashLoader_State_T * p_state = p_xfer->p_Substate;

    if (p_xfer->p_RxMeta->Id == p_app->WRITE_ID)
    {
        return Protocol_FlashLoaderDirect_Open(p_app, p_state, p_xfer, p_rxPayload, p_txPayload);
    }

    /* The ack of the opening reply. Nothing to absorb. */
    if (p_xfer->p_RxMeta->Id != p_app->DATA_ID) { return PROTOCOL_REQ_AWAIT; }
    ...
    p_state->Status = (uint16_t)Flash_ContinueWrite_Blocking(p_app->P_FLASH, p_rxPayload, p_xfer->p_RxMeta->Length);
...
*/
/******************************************************************************/
static inline Protocol_ReqCode_T Protocol_FlashLoaderDirect_Erase_Blocking(void * p_context, Packet_Xfer_T * p_xfer, const void * restrict p_rxPayload, void * restrict p_txPayload)
{
    Protocol_FlashLoader_T * p_app = p_context;
    const Protocol_FlashLoader_Req_T * p_req = p_rxPayload;

    ((Protocol_FlashLoader_Resp_T *)p_txPayload)->Status =
        (p_xfer->p_RxMeta->Length < sizeof(Protocol_FlashLoader_Req_T))
      ? PROTOCOL_FLASH_LOADER_STATUS_MALFORMED
      : (uint16_t)Flash_Erase_Blocking(p_app->P_FLASH, (uintptr_t)p_req->Address, (size_t)p_req->Size);
...
/*
    ...
    P_REQ_CONTEXT must be at least sizeof(Protocol_FlashLoader_State_T), and P_APP_CONTEXT is
    the interface above.
*/
/******************************************************************************/
