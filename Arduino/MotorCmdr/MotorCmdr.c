...
extern const Packet_Class_T MOTOR_CMDR_MOT_PROTOCOL_SPECS;  /* circular extern defined in MotorCmdr_MotProtocol */
...
void MotorCmdr_Init(MotorCmdr_T * p_motorCmdr)
{
    ...
    p_motorCmdr->Protocol.Config.WatchdogTimeout = 500U;
...
void MotorCmdr_SaveNvm(MotorCmdr_T * p_motorCmdr)         { Protocol_Cmdr_StartReq(&p_motorCmdr->Protocol, MOT_PACKET_SAVE_CONFIG); }
...
void MotorCmdr_WriteBrake(MotorCmdr_T * p_motorCmdr, uint16_t brake)
{
    ...
    p_motorCmdr->MotorWriteVarId = VEHICLE_VAR_BRAKE;
...
void MotorCmdr_WriteThrottle(MotorCmdr_T * p_motorCmdr, uint16_t throttle)
{
    ...
    MotorCmdr_WriteVar(p_motorCmdr, VEHICLE_VAR_THROTTLE, throttle);
}
...
//     // Protocol_Cmdr_StartReq(&p_motorCmdr->Protocol, MOT_PACKET_CONTROL_TYPE);
//     MotorCmdr_WriteVar(p_motorCmdr, VEHICLE_VAR_THROTTLE, 0U);
// }
...
// uint8_t _MotorCmdr_SaveNvm(MotorCmdr_T * p_motorCmdr)         { return _Protocol_Cmdr_BuildTxReq(&p_motorCmdr->Protocol, MOT_PACKET_SAVE_CONFIG); }
