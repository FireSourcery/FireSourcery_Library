...
static State_T * Stop_InputDirection(const Motor_T * p_motor, state_input_value_t direction)
{
    ...
    Motor_FOC_SetDirection(p_motor->P_ACTIVE, direction);
...
{
    assert((p_motor->P_ACTIVE->Direction != MOTOR_DIRECTION_NULL)); /* Set on exit stop */
    Phase_Float(&p_motor->PHASE);
    _StateMachine_EndSubState(p_motor->STATE_MACHINE.P_ACTIVE);
    Motor_FOC_ClearFeedbackState(p_motor->P_ACTIVE);
...
static void Passive_Proc(const Motor_T * p_motor)
{
    ...
    Motor_PollCaptureSensor(p_motor->P_ACTIVE); /* alternatively move to thread */
    Motor_FOC_ProcCaptureAngleVBemf(p_motor->P_ACTIVE);
}
...
static State_T * Passive_Next(const Motor_T * p_motor)
{
    ...
    // return (Motor_GetSpeed(p_motor->P_ACTIVE) == 0) ? &MOTOR_STATE_STOP : NULL;
...
static State_T * Passive_InputControl(const Motor_T * p_motor, state_input_value_t phaseOutput)
{
    ...
    switch ((Phase_Output_T)phaseOutput)
    {
        ...
        case PHASE_OUTPUT_V0:
            ...
            // if (Motor_GetSpeed(p_motor->P_ACTIVE) < ) { Phase_ActivateOutputV0(&p_motor->PHASE); }
        ...
        case PHASE_OUTPUT_VPWM:
            ...
            {
                // MotorSensor_Zero(p_motor->P_ACTIVE->p_ActiveSensor);
                ...
                else if (Motor_GetSpeed(p_motor->P_ACTIVE) == 0U) /* OpenLoop start at 0 speed */
                {
...
static State_T * Passive_InputDirection(const Motor_T * p_motor, state_input_value_t direction)
{
    ...
    if (Motor_GetSpeed(p_motor->P_ACTIVE) == 0U)
    {
        ...
        // }
        // Motor_FOC_SetDirection(p_motor->P_ACTIVE, (Motor_Direction_T)direction);
...
static void OpenLoop_Entry(const Motor_T * p_motor)
{
    ...
    FOC_ClearCaptureState(&p_motor->P_ACTIVE->Foc);
