...
static inline Motor_T * Motor_Table_ContextAt(Motor_Table_T * p_table, uint8_t motorIndex) { return &(p_table->P_DEVS[motorIndex]); }
...
{
    bool isEvery = true;
    for (uint8_t iMotor = 0U; iMotor < p_table->LENGTH; iMotor++) { if (Motor_GetUserDirection(p_table->P_DEVS[iMotor].P_MOTOR_STATE) != sign) { isEvery = false; break; } }
    return isEvery;
}
