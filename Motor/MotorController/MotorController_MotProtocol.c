...
/*! Control */
/******************************************************************************/
static void Req_Control(MotorController_T * p_app, MotPacket_ControlResp_T * p_txPacket, size_t * p_txSize, const MotPacket_ControlReq_T * p_rxPacket, size_t rxSize)
{
	(void)p_txPacket;
	(void)rxSize;
	Motor_T * p_motor = MotorController_GetPtrMotor(p_app, 0U);

	switch(p_rxPacket->CmdControl.ControlId) 	// MotPacket_ControlReq_ParseId(p_monitorId,  p_rxPacket);
	{
		case MOT_PROTOCOL_CONTROL_THROTTLE:		Motor_User_SetThrottleCmd(p_motor, p_rxPacket->CmdControl.ValueU16s[0U]); 	break;
		case MOT_PROTOCOL_CONTROL_BRAKE:		Motor_User_SetBrakeCmd(p_motor, p_rxPacket->CmdControl.ValueU16s[0U]); 		break;
		default: break;
	}

	*p_txSize = MotPacket_ControlResp_MainStatus_Build(p_txPacket, MOT_PROTOCOL_STATUS_RESP_OK);
...
/*! Monitor */
/******************************************************************************/
// static void TxPacket_BuildMonitorAdcBatchMsb(MotorController_T * p_app, MotPacket_MonitorResp_T * p_txPacket)
// {
...
static void Req_Monitor(MotorController_T * p_app, MotPacket_MonitorResp_T * p_txPacket, size_t * p_txSize, const MotPacket_MonitorReq_T * p_rxPacket, size_t rxSize)
{
	(void)rxSize;
	Motor_T * p_motor = MotorController_User_GetPtrMotor(p_app, 0U);
	...
	switch(p_rxPacket->CmdMonitor.MonitorId)
	{
		...
		case MOT_PROTOCOL_MONITOR_I_PHASES: 	*p_txSize = MotPacket_MonitorResp_IPhases_Build(p_txPacket, FOC_GetIa(&p_motor->Foc), FOC_GetIb(&p_motor->Foc), FOC_GetIc(&p_motor->Foc));		break;
...
static void Req_StopMotor(MotorController_T * p_app, MotPacket_T * p_txPacket, size_t * p_txSize, const MotPacket_Sync_T * p_rxPacket, size_t rxSize)
{
	...
	// *p_txSize = MotPacket_BuildHeader(p_txPacket, MOT_PROTOCOL_STOP_MOTORS, 0U);
	*p_txSize = 0U; //todo
}
...
static const Protocol_Req_T REQ_TABLE[] =
{
	...
	PROTOCOL_REQ_DEFINE(MOT_PROTOCOL_STOP_MOTORS, 		Req_StopMotor, 	0U, 	PROTOCOL_SYNC_ID_DISABLE ),
