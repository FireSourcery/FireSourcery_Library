...
{
    (void)argc; (void)argv;
    MotorController_ForceDisableControl(p_mc);
...
{
    Motor_Context_T * p_motor = MotorController_GetPtrMotor(p_mc, 0U);
    char * p_end;
    uint32_t value;
    ...
    if(argc == 1U) /* run */
    {
        MotorController_SetCmdThrottle(p_mc, 65536U / 20U);
    }
    else if(argc == 2U) /* run [num] */
    {
        ...
        else
        {
            ...
            MotorController_SetCmdThrottle(p_mc, value);
        }
    }
    else if(argc == 3U) /* run [throttle/brake] [num] */
    {
        value = strtoul(argv[2U], &p_end, 10);
        if      (strncmp(argv[1U], "throttle", 9U) == 0U)   { MotorController_SetCmdThrottle(p_mc, value); }
        else if (strncmp(argv[1U], "brake", 6U) == 0U)      { MotorController_SetCmdBrake(p_mc, value); }
    }
...
static void PrintSensor(Terminal_T * p_term, Motor_Context_T * p_motor)
{
    switch(Motor_GetSensorMode(p_motor))
    {
...
static Cmd_Status_T Cmd_rev_Proc(MotorController_T * p_mc)
{
    ...
    if(p_motor->JogIndex < Motor_GetPolePairs(p_motor) * 6U + 1U)
    {
...
static Cmd_Status_T Cmd_monitor_Proc(MotorController_T * p_mc)
{
    ...
    {
        case 0U:
            Terminal_SendString(p_term, "Speed: "); Terminal_SendNum(p_term, Motor_GetSpeed_Rpm(p_motor)); Terminal_SendString(p_term, " RPM, ");
            Terminal_SendNum(p_term, Motor_User_GetSpeed_Fract16(p_motor)); Terminal_SendString(p_term, " Fract16\r\n");
            ...
            Terminal_SendNum(p_term, Encoder_ModeDT_GetScalarVelocity(&p_motor->Encoder)); Terminal_SendString(p_term, " Fract16\r\n");
            ...
            Terminal_SendString(p_term, "Iq: "); Terminal_SendNum(p_term, p_motor->Foc.Iq); Terminal_SendString(p_term, ", Vq: "); Terminal_SendNum(p_term, p_motor->Foc.Vq); Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "Id: "); Terminal_SendNum(p_term, p_motor->Foc.Id); Terminal_SendString(p_term, ", Vd: "); Terminal_SendNum(p_term, p_motor->Foc.Vd); Terminal_SendString(p_term, " Fract16\r\n");

            // Terminal_SendString(p_term, "Throttle: ");     Terminal_SendNum(p_term, MotAnalogUser_GetThrottle(&p_mc->AnalogUser)); Terminal_SendString(p_term, " Fract16\r\n");
            // Terminal_SendString(p_term, "Brake: ");     Terminal_SendNum(p_term, MotAnalogUser_GetBrake(&p_mc->AnalogUser)); Terminal_SendString(p_term, " Fract16\r\n");
            ...
            switch(Motor_GetStateId(p_motor))
            {
                case MOTOR_STATE_ID_INIT:         Terminal_SendString(p_term, "Init");        break;
                case MOTOR_STATE_ID_STOP:         Terminal_SendString(p_term, "Stop");        break;
                case MOTOR_STATE_ID_OPEN_LOOP:    Terminal_SendString(p_term, "OpenLoop");    break;
                case MOTOR_STATE_ID_RUN:          Terminal_SendString(p_term, "Run");         break;
                case MOTOR_STATE_ID_FREEWHEEL:    Terminal_SendString(p_term, "Freewheel");   break;
                case MOTOR_STATE_ID_CALIBRATION:  Terminal_SendString(p_term, "Calib");       break;
                case MOTOR_STATE_ID_FAULT:        Terminal_SendString(p_term, "Fault");       break;
                default: break;
            }
            Terminal_SendString(p_term, "\r\n");

            Terminal_SendString(p_term, "MCSM: ");
            switch(MotorController_GetStateId(p_mc))
            {
                case MC_STATE_ID_INIT:    Terminal_SendString(p_term, "Init");    break;
                case MC_STATE_ID_STOP:    Terminal_SendString(p_term, "Stop");    break;
                #ifdef MOTOR_CONTROLLER_SERVO_ENABLE
                case MC_STATE_ID_SERVO:   Terminal_SendString(p_term, "Servo");   break;
                #endif
                case MC_STATE_ID_RUN:     Terminal_SendString(p_term, "Run");     break;
                case MC_STATE_ID_FAULT:   Terminal_SendString(p_term, "Fault");   break;
                default: break;
            }
            Terminal_SendString(p_term, "\r\n");
            ...
            // Terminal_SendString(p_term, "ElecAngle: "); Terminal_SendNum(p_term, Motor_GetElectricalAngle(p_motor)); Terminal_SendString(p_term, " Deg16\r\n");
            // Terminal_SendString(p_term, "MechAngle: "); Terminal_SendNum(p_term, Motor_GetMechanicalAngle(p_motor)); Terminal_SendString(p_term, " Deg16\r\n");
        ...
        case 2U:
            ...
            Terminal_SendNum(p_term, Motor_GetElectricalPower_VA(p_motor));        Terminal_SendString(p_term, " Volt-Amps, ");
            Terminal_SendNum(p_term, Motor_GetElectricalPower_UFract16(p_motor));   Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "VPhase: ");
            Terminal_SendNum(p_term, Motor_GetVPhase_Volts(p_motor));      Terminal_SendString(p_term, " Volts, ");
            Terminal_SendNum(p_term, Motor_GetVPhase_UFract16(p_motor));    Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "IPhase: ");
            Terminal_SendNum(p_term, Motor_GetIPhase_Amps(p_motor));       Terminal_SendString(p_term, " Amps, ");
            Terminal_SendNum(p_term, Motor_GetIPhase_UFract16(p_motor));    Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "IPeakAdcu: "); Terminal_SendNum(p_term, Linear_ADC_AdcuOfFract16(&p_motor->UnitsIa, FOC_GetIMagnitude(&p_motor->Foc)) - p_motor->Config.IaZeroRef_Adcu); Terminal_SendString(p_term, " ADCU\r\n");
        ...
        case 3U:
            // Terminal_SendString(p_term, "Bemf Peak: ");      Terminal_SendNum(p_term, Motor_GetVPhase_UFract16(p_motor)); Terminal_SendString(p_term, " Fract16\r\n");
            // Terminal_SendString(p_term, "Capture: ");        Terminal_SendNum(p_term, p_motor->VBemfPeak_Adcu); Terminal_SendString(p_term, " ADCU\r\n");
            // Terminal_SendString(p_term, "FOC: ");            Terminal_SendNum(p_term, Linear_ADC_AdcuOfFract16(&p_motor->UnitsVa, FOC_GetIMagnitude(&p_motor->Foc)) - p_motor->Config.IaZeroRef_Adcu); Terminal_SendString(p_term, " ADCU\r\n");
            // Terminal_SendString(p_term, "FOC Clarke: ");     Terminal_SendNum(p_term, Linear_ADC_AdcuOfFract16(&p_motor->UnitsVa, FOC_GetIMagnitude_Clarke(&p_motor->Foc)) - p_motor->Config.IaZeroRef_Adcu); Terminal_SendString(p_term, " ADCU\r\n");

        case 4U:
            // Terminal_SendString(p_term, "VectorMagnitude: "); Terminal_SendNum(p_term, VectorMagnitude); Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "Throttle: ");  Terminal_SendNum(p_term, MotAnalogUser_GetThrottle(&p_mc->AnalogUser));     Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "Brake: ");     Terminal_SendNum(p_term, MotAnalogUser_GetBrake(&p_mc->AnalogUser));        Terminal_SendString(p_term, " Fract16\r\n");
            Terminal_SendString(p_term, "Speed: ");     Terminal_SendNum(p_term, Motor_GetSpeed_Rpm(p_motor)); Terminal_SendString(p_term, " RPM\r\n");
            Terminal_SendString(p_term, "Iq: "); Terminal_SendNum(p_term, p_motor->Foc.Iq); Terminal_SendString(p_term, ", Vq: "); Terminal_SendNum(p_term, p_motor->Foc.Vq); Terminal_SendString(p_term, " Fract16\r\n");
...
static Cmd_Status_T Cmd_mode(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U) /* display modes */
    {
        ...
        switch(Motor_GetCommutationMode(p_motor))
        {
        ...
        switch(Motor_GetSensorMode(p_motor))
        {
            case ROTOR_SENSOR_ID_HALL:    Terminal_SendString(p_term, "HALL");    break;
            case ROTOR_SENSOR_ID_ENCODER: Terminal_SendString(p_term, "ENCODER"); break;
            default: break;
        }
        Terminal_SendString(p_term, "\r\n");

        Terminal_SendString(p_term, "FeedbackMode: ");
        switch(Motor_GetDefaultFeedbackMode(p_motor))
        {
        ...
        switch(Motor_GetDirectionCalibration(p_motor))
        {
    ...
    {
        if      (strncmp(argv[1U], "foc", 4U) == 0U)        { Motor_SetCommutationMode(p_motor, MOTOR_COMMUTATION_MODE_FOC); }
        else if (strncmp(argv[1U], "sixstep", 8U) == 0U)    { Motor_SetCommutationMode(p_motor, MOTOR_COMMUTATION_MODE_SIX_STEP); }
        else if (strncmp(argv[1U], "voltage", 8U) == 0U)    { Motor_SetDefaultFeedbackMode(p_motor, MOTOR_FEEDBACK_MODE_CONSTANT_VOLTAGE); }
        else if (strncmp(argv[1U], "current", 8U) == 0U)    { Motor_SetDefaultFeedbackMode(p_motor, MOTOR_FEEDBACK_MODE_CONSTANT_CURRENT); }
        else if (strncmp(argv[1U], "speedv", 7U) == 0U)     { Motor_SetDefaultFeedbackMode(p_motor, MOTOR_FEEDBACK_MODE_CONSTANT_SPEED_VOLTAGE); }
        else if (strncmp(argv[1U], "speedi", 7U) == 0U)     { Motor_SetDefaultFeedbackMode(p_motor, MOTOR_FEEDBACK_MODE_CONSTANT_SPEED_CURRENT); }
        else if (strncmp(argv[1U], "fwdccw", 7U) == 0U)     { Motor_SetDirectionCalibration(p_motor, MOTOR_FORWARD_IS_CCW); }
        else if (strncmp(argv[1U], "fwdcw", 6U) == 0U)      { Motor_SetDirectionCalibration(p_motor, MOTOR_FORWARD_IS_CW); }
        ...
        {
            MotorController_SetBrakeMode(p_mc, TRACTION_BRAKE_MODE_VOLTAGE);
            Terminal_SendString(p_term, "VFreq Brake\r\n");
        }
        else if(strncmp(argv[1U], "brakei", 7U) == 0U)
        {
            MotorController_SetBrakeMode(p_mc, TRACTION_BRAKE_MODE_TORQUE);
...
static Cmd_Status_T Cmd_calibrate(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        if      (strncmp(argv[1U], "encoder", 8U) == 0U)    { Motor_ActivateCalibrationEncoder(p_motor); status = CMD_STATUS_PROCESS_LOOP; }
        else if (strncmp(argv[1U], "hall", 5U) == 0U)       { Motor_ActivateCalibrationHall(p_motor); status = CMD_STATUS_PROCESS_LOOP; }
        else if (strncmp(argv[1U], "adc", 4U) == 0U)        { Motor_ActivateCalibrationAdc(p_motor); status = CMD_STATUS_PROCESS_LOOP; }
    #if defined(MOTOR_SENSOR_SIN_COS_ENABLE)
        else if (strncmp(argv[1U], "sincos", 7U) == 0U) { Motor_ActivateCalibrationSinCos(p_motor); status = CMD_STATUS_PROCESS_LOOP; }
    #endif
    }
...
static Cmd_Status_T Cmd_calibrate_Proc(MotorController_T * p_mc)
{
    ...
    if(Motor_GetStateId(p_motor) == MOTOR_STATE_ID_STOP)
    {
        status = CMD_STATUS_PROCESS_END;
...
void PrintThermistorLimit(Terminal_T * p_term, HeatMonitor_State_T * p_thermistor)
{
...
static Cmd_Status_T Cmd_heat(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        ...
        Terminal_SendNum(p_term, MotorController_GetAdcu(p_mc, MOT_ADC_CHANNEL_HEAT_PCB)); Terminal_SendString(p_term, " ADCU, ");
        Terminal_SendNum(p_term, MotorController_GetHeatPcb_DegC(p_mc, 1U)); Terminal_SendString(p_term, " C\r\n");
    #if        defined(MOTOR_CONTROLLER_HEAT_MOSFETS_TOP_BOT_ENABLE)
        Terminal_SendString(p_term, "MOSFETs Top: ");
        Terminal_SendNum(p_term, MotorController_GetHeatMosfetsTop_DegC(p_mc, 1U));     Terminal_SendString(p_term, " C\r\n");
        Terminal_SendString(p_term, "MOSFETs Bot: ");
        Terminal_SendNum(p_term, MotorController_GetHeatMosfetsBot_DegC(p_mc, 1U));     Terminal_SendString(p_term, " C\r\n");
    #else
        Terminal_SendString(p_term, "MOSFETs: ");
        Terminal_SendNum(p_term, MotorController_GetAdcu(p_mc, MOT_ADC_CHANNEL_HEAT_MOSFETS)); Terminal_SendString(p_term, " ADCU, ");
        Terminal_SendNum(p_term, MotorController_GetHeatMosfets_DegC(p_mc, 1U)); Terminal_SendString(p_term, " C\r\n");
    #endif
        Terminal_SendString(p_term, "Motor0: ");
        Terminal_SendNum(p_term, Motor_GetAdcu(p_motor, PHASE_ADC_CHANNEL_HEAT)); Terminal_SendString(p_term, " ADCU, ");
        Terminal_SendNum(p_term, Motor_GetHeat_DegC(p_motor, 1U)); Terminal_SendString(p_term, " C\r\n");
    }
    else if(argc == 2U)
    {
...
static Cmd_Status_T Cmd_auser(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        ...
        Terminal_SendNum(p_term, MotorController_GetAdcu(p_mc, MOT_ADC_CHANNEL_THROTTLE));     Terminal_SendString(p_term, " Adcu ");
        Terminal_SendNum(p_term, MotAnalogUser_GetThrottle(&p_mc->AnalogUser));                     Terminal_SendString(p_term, " Fract16\r\n");
        Terminal_SendString(p_term, "Brake: ");
        Terminal_SendNum(p_term, MotorController_GetAdcu(p_mc, MOT_ADC_CHANNEL_BRAKE));     Terminal_SendString(p_term, " Adcu ");
        Terminal_SendNum(p_term, MotAnalogUser_GetBrake(&p_mc->AnalogUser));                         Terminal_SendString(p_term, " Fract16\r\n");
    }
...
// static const char * STR_VANALOG         = "V Sensor:\r\n";
...
//     memcpy(p_stringDest, STR_VANALOG, strlen(STR_VANALOG));     p_stringDest += strlen(STR_VANALOG);
...
#ifdef VMONITOR_STRING_FUNCTIONS_ENABLE
...
*/
size_t VMonitor_ToString_Verbose(VMonitor_State_T * p_vMonitor, char * p_stringBuffer, uint16_t unitVScalar)
{
    char * p_stringDest = p_stringBuffer;
    int32_t num;
    char numStr[16U];
...
static Cmd_Status_T Cmd_v(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        vSource = MotorController_GetVSource_V(p_mc, 1000U);
        vSense = MotorController_GetVSense_V(p_mc, 1000U);
        vAcc = MotorController_GetVAccs_V(p_mc, 1000U);
        battery = MotorController_GetBatteryCharge_Unit1000(p_mc);
...
static Cmd_Status_T Cmd_fault(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        MotorController_ToggleUserFault(p_mc);
        ...
        Terminal_SendString(p_term, "Pcb: ");         Terminal_SendNum(p_term, MotorController_GetFaultHeatPcb_DegC(p_mc, 1U));     Terminal_SendString(p_term, "\r\n");
        Terminal_SendString(p_term, "Mosfets: ");     Terminal_SendNum(p_term, MotorController_GetFaultHeatMosfets_DegC(p_mc, 1U));     Terminal_SendString(p_term, "\r\n");
        Terminal_SendString(p_term, "\r\n");
    }
...
static Cmd_Status_T Cmd_direction(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        if(strncmp(argv[1U], "fwd", 2U) == 0U) { MotorController_SetDirection(p_mc, MOTOR_CONTROLLER_DIRECTION_FORWARD); }
        else if(strncmp(argv[1U], "rev", 2U) == 0U) { MotorController_SetDirection(p_mc, MOTOR_CONTROLLER_DIRECTION_REVERSE); }
    }
    if(argc == 3U)
    {
        if(strncmp(argv[1U], "calfwd", 4U) == 0U)
        {
            if(strncmp(argv[2U], "ccw", 4U) == 0U) { Motor_SetDirectionCalibration(p_motor, MOTOR_FORWARD_IS_CCW); }
            else if(strncmp(argv[2U], "cw", 3U) == 0U) { Motor_SetDirectionCalibration(p_motor, MOTOR_FORWARD_IS_CW); }
        }
    }
...
static Cmd_Status_T Cmd_set(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        ...
        {
            MotorController_SetLoadDefault(p_mc, true);
            MotorController_SaveBootReg_Blocking(p_mc);
...
static Cmd_Status_T Cmd_save(MotorController_T * p_mc, int argc, char ** argv)
{
    (void)argc;
    (void)argv;
    MotorController_SaveConfig_Blocking(p_mc);
    return CMD_STATUS_SUCCESS;
}
...
static Cmd_Status_T Cmd_beep(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        MotorController_BeepN(p_mc, 500U, 500U, 1U);
    }
    else if(argc == 2U)
    {
        if(strncmp(argv[1U], "stop", 5U) == 0U) { MotorController_BeepStop(p_mc); }
        else if(strncmp(argv[1U], "start", 6U) == 0U) { MotorController_BeepStart(p_mc, 500U, 500U); }
        else
        {
            count = strtoul(argv[1U], 0U, 10);
            MotorController_BeepN(p_mc, 500U, 500U, count);
        }
    }
...
static Cmd_Status_T Cmd_kvspeed(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        ...
        Motor_SetVSpeedRef_Rpm(p_motor, rpm);
        rpm = Motor_GetSpeedVRef_Rpm(p_motor);

        Terminal_SendString(p_term, "\r\nSpeedV_Rpm: "); Terminal_SendNum(p_term, rpm);
        Terminal_SendString(p_term, " PWM: "); Terminal_SendNum(p_term, Motor_ConvertToVSpeed(p_motor, rpm));
        Terminal_SendString(p_term, " Fract16\r\n");

        Terminal_SendString(p_term, "RPM: "); Terminal_SendNum(p_term, rpm * 3U / 4U);
        Terminal_SendString(p_term, " PWM: "); Terminal_SendNum(p_term, Motor_ConvertToVSpeed(p_motor, rpm * 3U / 4U));
        Terminal_SendString(p_term, " Fract16\r\n");

        Terminal_SendString(p_term, "RPM: "); Terminal_SendNum(p_term, rpm / 2U);
        Terminal_SendString(p_term, " PWM: "); Terminal_SendNum(p_term, Motor_ConvertToVSpeed(p_motor, rpm / 2U));
        Terminal_SendString(p_term, " Fract16\r\n");

        Terminal_SendString(p_term, "RPM: "); Terminal_SendNum(p_term, rpm * 1U / 4U);
        Terminal_SendString(p_term, " PWM: "); Terminal_SendNum(p_term, Motor_ConvertToVSpeed(p_motor, rpm * 1U / 4U));
        Terminal_SendString(p_term, " Fract16\r\n");
    }
...
static void PrintIPeak(Terminal_T * p_term, uint16_t min_Adcu, int32_t min_Fract16, uint16_t max_Adcu, int32_t max_Fract16)
{
    Terminal_SendString(p_term, "Min: ");
    Terminal_SendNum(p_term, min_Adcu); Terminal_SendString(p_term, " ADCU ");
    Terminal_SendNum(p_term, min_Fract16); Terminal_SendString(p_term, " Fract16 ");
    Terminal_SendString(p_term, "Max: ");
    Terminal_SendNum(p_term, max_Adcu); Terminal_SendString(p_term, " ADCU ");
    Terminal_SendNum(p_term, max_Fract16); Terminal_SendString(p_term, " Fract16\r\n");
}
...
static Cmd_Status_T Cmd_ipeak(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        ...
        Motor_SetIPeakRef_Adcu_Debug(p_motor, zeroToPeak_Adcu);
        ...
        PrintIPeak(p_term, min_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIa, min_Adcu), max_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIa, max_Adcu));
        ...
        PrintIPeak(p_term, min_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIb, min_Adcu), max_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIb, max_Adcu));
        ...
        PrintIPeak(p_term, min_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIc, min_Adcu), max_Adcu, Linear_ADC_Fract16(&p_motor->UnitsIc, max_Adcu));
...
static Cmd_Status_T Cmd_ilimit(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 2U)
    {
        ilimit_Fract16 = strtoul(argv[1U], 0U, 10);
        Motor_SetILimitParam_Percent16(p_motor, ilimit_Fract16, ilimit_Fract16);
...
static Cmd_Status_T Cmd_version(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    Terminal_SendNum(p_term, MotorController_GetLibraryVersionIndex(3U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetLibraryVersionIndex(2U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetLibraryVersionIndex(1U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetLibraryVersionIndex(0U)); Terminal_SendString(p_term, "\r\n");

    Terminal_SendString(p_term, "Firmware: ");
    Terminal_SendNum(p_term, MotorController_GetMainVersionIndex(p_mc, 3U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetMainVersionIndex(p_mc, 2U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetMainVersionIndex(p_mc, 1U)); Terminal_SendString(p_term, " ");
    Terminal_SendNum(p_term, MotorController_GetMainVersionIndex(p_mc, 0U)); Terminal_SendString(p_term, "\r\n");
...
static Cmd_Status_T Cmd_servo(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    if(argc == 1U)
    {
        MotorController_EnterServoMode(p_mc);
        if(MotorController_GetStateId(p_mc) == MC_STATE_ID_SERVO) { Terminal_SendString(p_term, "Entered Servo Mode\r\n"); }
    ...
    else if(argc == 2U)
    {
        ...
        {
            MotorController_ExitServoMode(p_mc);
            if(MotorController_GetStateId(p_mc) != MC_STATE_ID_SERVO) { Terminal_SendString(p_term, "Exited Servo Mode\r\n"); }
            else { Terminal_SendString(p_term, "Failed\r\n"); }
        }
    }
#endif
...
static Cmd_Status_T Cmd_debug(MotorController_T * p_mc, int argc, char ** argv)
{
    ...
    // Terminal_SendString(p_term, "Manufacture:\r\n");
    // MotorController_GetManufacture(p_mc, &TestRead);
    ...
    // MotorController_WriteManufacture(p_mc, &TEST);
    ...
    // MotorController_WriteManufacture(p_mc, &TEST1);
    // Terminal_SendNum(p_term, p_mc->NvmStatus);Terminal_SendString(p_term, "\r\n");
    // MotorController_ReadOnce_Blocking(p_mc);
    // MotorController_GetManufacture(p_mc, &TestRead);
    // Terminal_SendString_Len(p_term, TestRead.NAME, 8U); Terminal_SendString(p_term, "\r\n");
