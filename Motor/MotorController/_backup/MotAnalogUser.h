...
{
    if (p_aIn->UseEdgePin == true) { Debounce_PollEdge(&p_aIn->EdgePin); }
    if ((p_aIn->UseEdgePin == false) || (Debounce_GetState(&p_aIn->EdgePin) == true))
    {
        p_aIn->ValuePrev_Percent16 = p_aIn->Value_Percent16;
        p_aIn->Value_Percent16 = ((uint32_t)Linear_ADC_Percent16(&p_aIn->Units, value_Adcu) + p_aIn->Value_Percent16) / 2U;
    }
}
...
static inline bool _MotAnalogUser_AIn_PollFallingEdge(MotAnalogUser_AIn_T * p_aIn)
{
    ...
    if(p_aIn->UseEdgePin == true)
    {
        ...
        else if(isValueFallingEdge == true)                     { p_aIn->EdgePin.DebouncedState = false; isFallingEdge = true; }
...
{
    Debounce_PollEdge(&p_user->ReversePin);
    if(p_user->Config.UseForwardPin == true)        { Debounce_PollEdge(&p_user->ForwardPin); }
    if(p_user->Config.UseNeutralPin == true)        { Debounce_PollEdge(&p_user->NeutralPin); }
...
extern void MotAnalogUser_ConfigId_Set(MotAnalogUser_T * p_user, MotAnalogUser_ConfigId_T id, int32_t value);