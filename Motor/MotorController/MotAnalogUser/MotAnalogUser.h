...
typedef struct MotAnalogUser_AIn_Tag
{
	...
	Linear_T Units;
	uint16_t Value_Frac16;
	uint16_t ValuePrev_Frac16;
}
MotAnalogUser_AIn_T;
...
#define MOT_ANALOG_USER_INIT(p_BrakePinHal, BrakePinId, p_ThrottlePinHal, ThrottlePinId, p_ForwardPinHal, ForwardPinId, p_ReversePinHal, ReversePinId, p_BistateBrakePinHal, BistateBrakePinId, p_ThrottleSafetyPinHal, ThrottleSafetyPinId, InputPinsInvert, p_Millis, p_Params)	\
{																																\
	...
	.ThrottleSafetyPin 		= DEBOUNCE_INIT(p_ThrottleSafetyPinHal, 	ThrottleSafetyPinId, 	InputPinsInvert, p_Millis), 	\
...
static inline void _MotAnalogUser_AIn_CaptureValue(MotAnalogUser_AIn_T * p_aIn, uint16_t value_Adcu)
{
	...
	{
		p_aIn->ValuePrev_Frac16 = p_aIn->Value_Frac16;
		// p_aIn->Value_Frac16 = Linear_ADC_CalcFractionUnsigned16(&p_aIn->Units, value_Adcu);
		p_aIn->Value_Frac16 = (Linear_ADC_CalcFractionUnsigned16(&p_aIn->Units, value_Adcu) + p_aIn->ValuePrev_Frac16) / 2U;
	}
}
...
{
	return (p_aIn->UseEdgePin == true) ? (Debounce_GetState(&p_aIn->EdgePin) && (p_aIn->Value_Frac16 > 0U)) : (p_aIn->Value_Frac16 > 0U);
}

static inline bool _MotAnalogUser_AIn_PollFallingEdge(MotAnalogUser_AIn_T * p_aIn)
{
	bool isValueFallingEdge = ((p_aIn->Value_Frac16 == 0U) && (p_aIn->ValuePrev_Frac16 > 0U));
	bool isFallingEdge;

	if(p_aIn->UseEdgePin == true)
	{
		/* Once 1 part detects falling edge, disable the other from repeat detect */
		if(Debounce_PollFallingEdge(&p_aIn->EdgePin) == true) 	{ p_aIn->Value_Frac16 = 0U; p_aIn->ValuePrev_Frac16 = 0U; isFallingEdge = true; }
	...
	// return (p_aIn->UseEdgePin == true) ? (Debounce_PollFallingEdge(&p_aIn->EdgePin)) : ((p_aIn->Value_Frac16 == 0U) && (p_aIn->ValuePrev_Frac16 > 0U));
}

static inline uint16_t _MotAnalogUser_AIn_GetValue(const MotAnalogUser_AIn_T * p_aIn)
{
	return (_MotAnalogUser_AIn_GetIsOn(p_aIn) == true) ? p_aIn->Value_Frac16 : 0U; /* Check IsOn again, If !IsOn Value_Frac16 remains prev captured value*/
}
...
// 	{
// 		if(p_user->BrakeAIn.Value_Frac16 < p_user->Params.BistateBrakeValue_Frac16) //using AIn.Value as shared value
// 		{
// 			p_user->BrakeAIn.Value_Frac16 = p_user->Params.BistateBrakeValue_Frac16;
// 		}
// 	}
// }
...
// static inline bool MotAnalogUser_GetIsThrottleOn(const MotAnalogUser_T * p_user) 		{ return _MotAnalogUser_AIn_GetIsOn(&p_user->ThrottleAIn); }
// static inline bool MotAnalogUser_GetIsBrakeOn(const MotAnalogUser_T * p_user) 			{ return _MotAnalogUser_AIn_GetIsOn(&p_user->BrakeAIn); }
// static inline bool MotAnalogUser_PollThrottleFallingEdge(MotAnalogUser_T * p_user) 		{ return _MotAnalogUser_AIn_PollFallingEdge(&p_user->ThrottleAIn); }
// static inline bool MotAnalogUser_PollBrakeFallingEdge(MotAnalogUser_T * p_user) 			{ return _MotAnalogUser_AIn_PollFallingEdge(&p_user->BrakeAIn); }
...
static inline MotAnalogUser_Cmd_T MotAnalogUser_PollCmd(MotAnalogUser_T * p_user)
{
	...
	switch(direction)
	{
		...
		default:
			/* Check Brake first */
			if((_MotAnalogUser_AIn_GetIsOn(&p_user->BrakeAIn) == true) || (MotAnalogUser_GetIsBistateBrakeOn(p_user) == true)) //p_user->BrakeAIn.Value_Frac16 > 0U
			{
...
static inline uint16_t MotAnalogUser_GetBrake(const MotAnalogUser_T * p_user)
{
	...
	if((MotAnalogUser_GetIsBistateBrakeOn(p_user) == true) && (p_user->Params.BistateBrakeValue_Frac16 > brakeValue))
	{
		brakeValue = p_user->Params.BistateBrakeValue_Frac16;
	}
	return brakeValue;
}
