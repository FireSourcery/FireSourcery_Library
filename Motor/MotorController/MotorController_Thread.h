...
static inline void MotorController_Main_Thread(MotorControllerPtr_T p_mc)
{
    ...
    if(Timer_Periodic_Poll(&p_mc->TimerMillis) == true)
    {
        ...
        if(CheckDividerMask(p_mc->MainDividerCounter, p_mc->CONFIG.MAIN_DIVIDER_1000) == true)
        {
            ...
            p_mc->VSource = MotorController_User_GetVSource_V(p_mc, 10U);
            p_mc->VSense = MotorController_User_GetVSense_V(p_mc, 1000U);
            p_mc->VAccs = MotorController_User_GetVAccs_V(p_mc, 1000U);
        #endif
        }
    }
}
