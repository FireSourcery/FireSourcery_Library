...
void Vehicle_VarId_Set(const Vehicle_T * p_vehicle, Vehicle_VarId_T id, int value)
{
    switch (id)
    {
        ...
        case VEHICLE_VAR_THROTTLE:    Vehicle_User_ApplyThrottle(p_vehicle->P_VEHICLE_STATE, (uint16_t)value);      break;
        case VEHICLE_VAR_BRAKE:       Vehicle_User_ApplyBrake(p_vehicle->P_VEHICLE_STATE, (uint16_t)value);         break;
    }
}
