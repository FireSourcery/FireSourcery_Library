...
{
	volatile void * P_BUFFER;
	size_t LENGTH; 	/* in unit counts */
	size_t UNIT_SIZE;
...
static inline size_t CalcQueueIndexMasked(Queue_T * p_queue, size_t index)
{
...
static inline size_t CalcQueueIndexInc(Queue_T * p_queue, size_t index, size_t inc)
{
...
static inline size_t CalcQueueIndexDec(Queue_T * p_queue, size_t index, size_t dec)
{
#if defined(CONFIG_QUEUE_LENGTH_POW2) || defined(CONFIG_QUEUE_LENGTH_POW2_INDEX_UNBOUNDED)
 ...
 */
static inline size_t Queue_GetFullCount(Queue_T * p_queue)
{
 ...
 */
static inline size_t Queue_GetEmptyCount(Queue_T * p_queue)
{
#ifdef CONFIG_QUEUE_LENGTH_POW2_INDEX_UNBOUNDED
	return CalcQueueIndexMasked(p_queue, p_queue->Tail - p_queue->Head);
#elif defined(CONFIG_QUEUE_LENGTH_POW2)
	return CalcQueueIndexMasked(p_queue, p_queue->Tail - p_queue->Head - 1U);
#else
...
static inline bool Queue_GetIsEmpty(Queue_T *p_queue)
{
	return (p_queue->Head == p_queue->Tail);
}

static inline bool Queue_GetIsFull(Queue_T * p_queue)
{
#ifdef CONFIG_QUEUE_LENGTH_POW2_INDEX_UNBOUNDED
...
//extern Queue_T * Queue_New(size_t bufferLength, size_t unitSize);
...
//extern bool Queue_EnqueueBytes(Queue_T * p_queue, const void * p_data, size_t n);
//extern bool Queue_DequeueBytes(Queue_T * p_queue, void * p_dest, size_t n);
